...
| R8.4 | Migrate persistence, share links, swap, presets and Matrix workers from `unitType` to the R8 identity model without breaking old saved states. Test predefined locking and Custom editing; version changes; roster/template and hero-type fidelity; base-versus-live Fantastic/race gates; every Special unit option; Construct Catapult with and without Combat Summoned; ordered realm overrides such as Chosen versus Chaos Channels/Undead/No Heal/Sanctify; and main-thread/worker parity. | open | medium | persistence UI contract; `ui.js`; `combat.js`; worker state; R7 trace |
