...
definePresets({
  ...
  militaryWorkshopMeleeOnlyExcludedWarlord: {
    ...
    vacuity: {
      ...
      'a.ability.militaryWorkshop':
        'Keep, and the absence is the rule under test. Unlike the hero term beside it, this exclusion is the script\'s own: the Workshop block\'s inner gate is `(SRanged>0 %AND SRangedType<30) %OR SThrown>0 %OR SFireBreath>0` (CreateUnit.CAS!NOTGENERIC!+42..+44 "IF ( (GetStat(U,SRanged,1)>0) %AND (GetStat(U,SRangedType,1)<30) )" "%OR (GetStat(U,SFireBreath,1)>0)"), so a unit with none of the three reaches neither the `SBlackpowderUpgrade` write at CreateUnit.CAS!NOTGENERIC!+46 "SETSTAT(U,SBlackpowderUpgrade,1,1);" nor the `AFPoison` increment at CreateUnit.CAS!NOTGENERIC!+47 "IF (GetStat(U,AFPoison,1,1)<>100) THEN {". The calculator carries it as `blackpowderEligibleAttack` (stats.js:1175-1176), a term of `blackpowder` (stats.js:1183-1184); A states no `modernAttacks` at all, so no channel is marked and `base:militaryWorkshop`\'s `when` is false (stats_sequence.js:157). The poison half is asserted too: the defender is deliberately left at res 1 with no Poison Immunity, where an erroneous +1 Poison grant would score a full 1.0 (poisonFailProb, combat_special_attacks.js:19) instead of this 0. militaryWorkshopArmorPiercingWarlord is the eligible arm at 6.000, but it is not a numeric contrast to this fixture: it swaps the melee attack for a boulder and gives b def 8 and Poison Immunity.',
    },
  },
  militaryWorkshopHeroExcludedWarlord: {
    ...
    vacuity: {
      'a.ability.militaryWorkshop':
        'Keep, and the absence is the rule under test: `baseNormalTrainingUnit` is `!isHero && !isFantasticBase` (stats.js:320) and it is a term of `blackpowder` (stats.js:1183-1184), so on a hero no channel is marked, `base:militaryWorkshop`\'s `when` is false (stats_sequence.js:157), and neither the Armor Piercing grant nor the `<>100` poison increment lands. That hero term is the calculator\'s own, stated at stats.js:299-305 from the changelog\'s "base normal units" wording (`Reference docs/Warlord manual v1.5.12.9.html`). The script makes no hero test: CreateUnit.CAS contains no `ISHERO` anywhere, checked case-insensitively over the whole file. Its Workshop block gates only on the city or Rocketry (CreateUnit.CAS!NOTGENERIC!+38..+40 "IF (ISBUILT(C,BMilitaryWorkshop)=0)" "THEN { GOTO") and then on the attack-presence test at CreateUnit.CAS!NOTGENERIC!+42..+44 "IF ( (GetStat(U,SRanged,1)>0) %AND (GetStat(U,SRangedType,1)<30) )" "%OR (GetStat(U,SFireBreath,1)>0)", and carries no unit-race term either - a block-level negative, since the file does test the unit\'s own `SRace` at CreateUnit.CAS!NOLOGISTIC!+50 "IF (GETSTAT(U,SRace,1)<>RCGeneric) %OR (GETSTAT(U,STypeID,1)=44) THEN { GOTO", in the generic-to-racial conversion. a.unitType=hero is the live half. The defender deliberately carries no Poison Immunity, so an erroneous poison grant would surface in the total rather than be absorbed. militaryWorkshopArmorPiercingWarlord is the normal-unit arm at 6.000 against this 2.000; it is not a one-value sibling, since it also gives b Poison Immunity.',
    },
  },
  ...
  rebuildMechanicalTooLateForArtificerWarlord: {
    ...
    vacuity: {
      'a.ability.artificer':
        'Keep, and the absence is the rule under test - the claim is exactly that the retort cannot see the flag Rebuild writes. `base:artificer` reads the record at its own rank (`when: u => !!u.mechanical`, combat_abilities.js:1299), from `GetStat(U,SCustomAttribute,1)=1` at CreateUnit.CAS!NOLOGISTIC!+8 "IF RETORT(W,Artificer) %AND (GetStat(U,SCustomAttribute,1)=1) THEN {", and the chain places `base:artificer` among the training-time writes (stats_manifests.js:243) while `base:rebuild`, whose apply sets `u.mechanical = true` (combat_abilities.js:1359), is cast-time and strictly later (stats_manifests.js:254). The gate therefore reads false and the retort\'s `u.atk += 1; u.def += 1; u.res += 2;` (:1305) and +1 ranged (:1306) all stay away. Its Magic Weapons half is closed by a second, independent guard, so a change to the rank alone would not move this number: `artificerMagicWeapon` reads the two raw input flags (stats.js:157-158) and this fixture states no `mechanical`. a.ability.rebuild is the live half. mechanicalExpertRebuildMakesMechanicalWarlord is the later reader that does see the write, four regions on.',
    },
  },
});
