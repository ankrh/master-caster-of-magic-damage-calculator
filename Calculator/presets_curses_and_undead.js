...
definePresets({
  ...
  bombsGrenadesSapiensLabelKeepsGrantThroughApotheosisWarlord: {
    desc: 'The `SMultiLabel = 14` arm of the same gate, which bombsGrenadesSkipsApotheosisPermanentFantasticWarlord names and does not measure. `IF (BASEFANTASTIC(U)>0) %AND (GETSTAT(U,SMultiLabel,1)<>14) THEN { GOTO "NOTSAPIENS"; }` (UnitCalcPre.CAS!NOMAGITEKENGINE!+2..+4 "IF (BASEFANTASTIC(U)>0)" "THEN { GOTO") is a conjunction, so a labelled unit keeps the grant however Fantastic the permanent record is. Selector 1 is the base unit (CAS reference, Scripts.TXT:270), so the label is a permanent-record field like the flag beside it (F263). Apotheosis writes `B.Fantastic := True` at $0059A390 and the Sapiens ability supplies the label: melee 1 doubled by Apotheosis is 2, Thrown floor(8 - 4/2) = 6 doubled to 12, and 4 figures at 100% hit against 0 Defense deal (2 + 12) x 4 = 56.0. Dropping the Sapiens ability is exactly the sibling fixture and leaves 8.0; dropping Apotheosis leaves the undoubled 28.0.',
    version: V_WARLORD,
    a: { figs:4, atk:1, hitChance:70, hp:10,
      abilities: { outlanderWizard: true, explosive: true, apotheosis: true, sapiens: true } },
    b: { def:0, toBlkMod:70, hp:60 },
    expected: { dmgToA: 0, sdDmgToA: 0.000, dmgToB: 56.000, sdDmgToB: 0.000 },
  },
  bombsGrenadesSpiritLinkLabelSurvivesApotheosisWarlord: {
    desc: 'Spirit Link is the second writer of the label — `SETSTAT(TU,SMultiLabel,1,14)` (OLSpell.CAS!NOTAIRSUPPORT!+8 "SETSTAT(TU,SMultiLabel,1,14);"), the first of three writes under the `IF BASEFANTASTIC(TU)` the block carries (OLSpell.CAS!NOTAIRSUPPORT!+7 "IF BASEFANTASTIC(TU) THEN {") — and the label is not redundant with the Fantastic clear beside it, because the `B.Fantastic := True` Apotheosis makes ($0059A390) is re-made on every recalculation pass and does not touch `SMultiLabel`. So a base-Fantastic unit given both is Fantastic to the `NOTSAPIENS` gate and still labelled 14, and the conjunction admits it: melee 1 doubled is 2, Thrown floor(8 - 4/2) = 6 doubled to 12, (2 + 12) x 4 = 56.0. Dropping Spirit Link leaves the closed gate at 8.0; dropping Apotheosis leaves the undoubled 28.0, which the clear alone would also give.',
    version: V_WARLORD,
    a: { figs:4, atk:1, hitChance:70, hp:10, unitType: 'fantastic_chaos',
      abilities: { outlanderWizard: true, explosive: true, apotheosis: true, spiritLink: true } },
    b: { def:0, toBlkMod:70, hp:60 },
    expected: { dmgToA: 0, sdDmgToA: 0.000, dmgToB: 56.000, sdDmgToB: 0.000 },
  },
  bombsGrenadesSpiritLinkLabelNeedsBaseFantasticWarlord: {
    desc: 'The other side of the gate that write carries. `SETSTAT(TU,SMultiLabel,1,14)` stands inside `IF BASEFANTASTIC(TU)` (OLSpell.CAS!NOTAIRSUPPORT!+7 "IF BASEFANTASTIC(TU) THEN {"), so Spirit Link cast on a unit whose permanent record is not Fantastic at that rank writes no label at all. Negative claim, and the absence is the rule under test: this unit is base-normal, so the cast leaves the label unwritten, Apotheosis then makes the permanent record Fantastic, and the `NOTSAPIENS` gate closes on a unit that has no exemption — melee 1 doubled by Apotheosis is 2 over 4 figures at 100% hit vs 0 Defense = 8.0, the same as bombsGrenadesSkipsApotheosisPermanentFantasticWarlord. Writing the label without the gate would grant Thrown 6, doubled to 12, for 56.0; making the unit base-Fantastic instead is bombsGrenadesSpiritLinkLabelSurvivesApotheosisWarlord and reads exactly that.',
    version: V_WARLORD,
    a: { figs:4, atk:1, hitChance:70, hp:10,
      abilities: { outlanderWizard: true, explosive: true, apotheosis: true, spiritLink: true } },
    b: { def:0, toBlkMod:70, hp:60 },
