...
// Each is UNVERIFIED, and the gap is the write rather than the gate. No supported source in this
// tree reconstructs a curse cast writing its own flag onto the permanent record —
...
function curseCastSteps(version, marked, eyeOfHeaven) {
  ...
  const cast = key => ({ sourceId: key, sourceLabel: CURSE_CAST_LABELS[key], writes: [key],
    ...
    apply: u => { u[key] = true; } });
  return [
    // PROVENANCE[weakness:cast]: UNVERIFIED versions=mom_1.31,mom_cp_1.60.00,com_6.08,com2_1.05.11,com2_warlord_1.5.12.9; gap=F250.2 first combat subunit reconstructs modern generic combat-region stores, not base/permanent stores. INI-to-runtime field loading, complete named admission, scripts and DOS evidence remain open, so this permanent-record step is not verified.; pointer=Reference docs/Caster binary/F250.2.combat.evidence.md
    ...
    // PROVENANCE[warpAttack:cast]: UNVERIFIED versions=mom_1.31,mom_cp_1.60.00,com_6.08,com2_1.05.11,com2_warlord_1.5.12.9; gap=F250.2 first combat subunit maps WarpCreatureEffect to combat-region indices 6/7/8, not base/permanent flags. Caller admission, later scripts, persistence and DOS writes remain open, so this permanent-record step is not verified.; pointer=Reference docs/Caster binary/F250.2.combat.evidence.md
