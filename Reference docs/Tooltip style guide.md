...
**Warlord is a separate version token.** It is *not* covered by `CoM 1 & 2`, even though the Warlord mod is built on CoM2. Every tooltip for a mechanic that exists in Warlord must address Warlord explicitly — use `CoM 1 & 2 & Warlord:` when the value is shared (verify against `Unit rosters/Warlord mod unit data/HELP.TXT` / `Warlord manual v1.5.12.9.html`, and against `Reference docs/Script source/Warlord 1.5.12.9/` where the mechanic is scripted), or a separate `Warlord:` line when it differs. If Warlord behavior is unverified, flag it rather than folding it into the CoM2 line.
...
## Calculated-value hover chains

A different kind of tooltip, and not an authored string: the chain a calculated output shows on
hover is built from the ordered transform path that computed it, and only its wording is decided
here. One line per transform, `Label (phase X): from → to`, between an `Editable base:` line and
a `Displayed result:` line.

A rider histogram inside a phase row carries the chains of the figures its roll was scored
against, each under a heading naming what was computed and whose it is:

```
Effective Resistance (defender) vs Nature
Effective Resistance (defender), realm-less roll
Effective Defense (attacker)
```

The realm is not decoration. One unit has one effective resistance per realm its attacker's
riders name, and up to five of them stand at once inside a single attack — Life for Dispel Evil
and Exorcise, Nature for Stoning Touch, Death for Death Touch and Life Steal, Chaos for
Destruction, and no realm at all for Poison. Two of them are often the same number and are still
answers to different questions, so a chain without its realm is unreadable. The subject is named
for the same reason: Life Steal's healing histogram is plotted on the attacker while the roll
behind it was made against the defender.
