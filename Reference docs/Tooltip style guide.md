...
**Warlord is a separate version token.** It is *not* covered by `CoM 1 & 2`, even though the Warlord mod is built on CoM2. Every tooltip for a mechanic that exists in Warlord must address Warlord explicitly — use `CoM 1 & 2 & Warlord:` when the value is shared (verify against `Unit rosters/Warlord mod unit data/HELP.TXT` / `Warlord manual v1.5.12.7.html`, and against `Reference docs/Script source/Warlord 1.5.12.7/` where the mechanic is scripted), or a separate `Warlord:` line when it differs. If Warlord behavior is unverified, flag it rather than folding it into the CoM2 line.
