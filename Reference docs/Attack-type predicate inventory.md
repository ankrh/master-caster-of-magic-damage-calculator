...
| E64 | the material block's strength writes (`stats.js`, `weaponStatSteps`) | — | `c:weapon` | `Inc(Units[i].ranged, j)` at `:653` (no strength gate) and `Inc(Units[i].thrown, j)` at `:661` (`Units[i].thrown > 0`) | live since [F97](../Calculator/HISTORY.md) | match on the modern arms since F97: the ranged write makes no strength test and the Thrown write reads the calculated field. The DOS arm keeps its fabricated `calcBaseRtb > 0` on both halves, where `unitcalc.c` 131:0x8F089 makes one type-only test and no strength test — [F109](../Calculator/BACKLOG.md), because removing it moves numbers in all three DOS versions |
...
| E72 | `flameBladeRangedStep`'s live type read | `:1319-1341` | `c:flameBlade:ranged` | `Units.RecalculateUnits.pas:1675-1687` (`$0059FE47`) on the modern engines, `unitcalc.c` 131:0x8F5A8 on the DOS ones — the same compiled block in all five | **live**, correctly, and the position is the engine's | match. The region-`b` doubt is closed: `UnitCalcPre.CAS:840-845` is Fiery Fury's own write, carried by `b:fieryFury`, and `EncFlameBlade` appears nowhere else in `UnitCalcPre.CAS`, so Warlord makes no blade ranged-strength write of its own. The compiled block runs after Focus Magic (`$0059A63B`) and reads the current `U.rangedtype`, which the conversion has set to 34 — so the blade's +2 is genuinely lost under Focus Magic, and region `c` is where the read belongs. Measured at region `b` for contrast: a 4-strength Warlord missile finishes 6 rather than 4. What remains under [F92](../Calculator/BACKLOG.md) is the [M11](../Calculator/HISTORY.md) c-pair merge, blocked on the step's Metal Fires branch ([F111](../Calculator/BACKLOG.md)) |
...
| `rtbLvl` collapses four base-record gates and three tables into one live if/else | E29, I2 | settled by [F95](../Calculator/HISTORY.md), which held the row's measurement in both halves and found `UNITS.INI [362]` Wanderer the one shipped record the ranged gate moves — filed as [F107](../Calculator/BACKLOG.md) |
...
| Weapon-material strength gates read the wrong record and add a gate the engine lacks | E39, E62, E64 | settled by [F97](../Calculator/HISTORY.md) for the modern arms, which measured 508 moving cases in `com2_1.05.11` and `com2_warlord_1.5.12.7` and none in the DOS versions, and found E62's To-Hit half already made one level up; the same fabricated gate on the DOS arm is [F109](../Calculator/BACKLOG.md) |
