...
| Corpus | Use |
|---|---|
| [`Caster binary/Units.RecalculateUnits.pas`](./Caster%20binary/Units.RecalculateUnits.pas) | CoM2 1.05.11 / Warlord 1.5.12.9 compiled control flow, including all 13 `Ismagicalranged` call sites and the `@Units@ApplyLevelBonus`, `@Units@ApplyMagicWeapons` and aura-pass bodies |
| [`Script source/Warlord 1.5.12.9/`](./Script%20source/Warlord%201.5.12.9/) | `CreateUnit.CAS`, `UnitCalcPre.CAS`, `UnitCalc.CAS`, `Levelbonus.INI`, `RangedType.INI` |
...
| E12 | `hasGazeAttack` | `:784-787` | `ccDosBreathEligibleAt` | DOS Chaos Channels admission | permanent | match. Kept on the slot context by [F244.3i](../TASKS.md): which mirrors of the shared byte the record carries is structure, and the two gaze abilities are card marks — neither is a value a step writes |
| E13 | `ccDosBreathEligibleAt` | `:789-790` | `c:chaosChannels:fireBreath`, and MoM 1.31's `:recompute` beside it | `unitcalc.c` DOS admission gate | permanent, and **read off `ctx.base`** since F244.3i rather than off the card's `rtb`/`rtbType` input | match. The single raw token became the record's two type fields, which is the shape the record stores it in; a gaze template leaves both `none` and is refused by `hasGazeAttack` |
...
| E16 | `hasPermanentRangedStat` | `:767` | `training:energyCannon`, `training:ludusAgoge`, `training:motherFungus`, `training:altarOfTheMoon`, `training:naturalSelection:wildGame` | `CreateUnit.CAS` city gates, which do read a permanent ranged type | permanent | match — the Misfortune consumer is gone with [F96](../Calculator/HISTORY.md). Every consumer left is a `training` step, which runs four phases ahead of `a:baseCopy` and so cannot read `ctx.base` at all; `slots.persistentRanged` and the record-level Energy Cannon gate left for `ctx.base` with F244.3i |
...
| E48 | `disciplineRtbMod` | `:988-989` | `c:discipline` | `not Ismagicalranged(U.rangedtype)` at `:1554` (I6) | live | **`narrow`** — new site for [F89](../Calculator/HISTORY.md). The `levelRankOf(u.level) >= 2` half is correct: the rank is zero-based where `U.level` is one-based, so it is the engine's `U.level >= 3`. |
...
|---|---|---|---|---|---|---|
| E59 | `rustRtbMod` | `stats.js:1056` | `d:rust` | `UnitCalc.CAS!NOTCITY!+11 "IF (GETENCHANTMENTFLAG(U,EncRust,0)=0) THEN { GOTO"` Rust | live | match |
...
| E69 | `slots.persistentRanged` | `:1651` | `e:mislead` (Misfortune aura), `e:holyBonus` | `if B.ranged > 0` at `:2599` and `:2535` | permanent, and **`ctx.base`** since F244.3i — a predicate over the run context, as `slots.melee` already was | match — [F96](../Calculator/HISTORY.md) made it the permanent record's Ranged **field** carrying strength: which field the slot is, with no type test on a modern channel. The DOS-shaped shared slot keeps its type test, since one value stands there for ranged, Thrown, Breath and a gaze alike |
...
that was the miss: `SETSTAT(U,SRangedType,0,...)` (`UnitCalcPre.CAS!NOVAMPIRISM!+26 "SETSTAT(U,SRangedType,0,37);"`,
...
`UnitCalcPre.CAS!NOTLIFEMARIONETTESPELL!+4 "SETSTAT(U,SRangedType,0,33);"`,
`UnitCalcPre.CAS!ENDOFMARIONETTESPELLSELECT!+80 "SETSTAT(U,SRangedType,0,30);"`) is record selector `0`, a region-`b` write on the *calculated* record, while
...
| Weapon-material strength gates read the wrong record and add a gate the engine lacks | E39, E62, E64 | settled by [F97](../Calculator/HISTORY.md) for the modern arms, which measured 508 moving cases in `com2_1.05.11` and `com2_warlord_1.5.12.9` and none in the DOS versions, and found E62's To-Hit half already made one level up; settled for the DOS arm by [F109](../Calculator/HISTORY.md), which measured the three DOS versions moving and both modern ones unchanged |
