...
// Version gating as state: the two named gates, and `applyVersionGating` as the one
// implementation of the clearing the page used to do inline (F260.3).
//
// Two things are asserted here that nothing else can see:
//
//   1. **The card and the matrix disagree, and by exactly how much.** The card gates a def with
//      `abilityGatedForCard` (subgroup plus the def's own `alsoVersions`/`exceptVersions`); the
//      matrix uses `abilityGatedForMatrix` (subgroup alone, plus a `blur` exception). They differ
//      on six `com2_warlord_1.5.12.9` enchantments, which is INV-2 in the matrix and is **F261's**
//      to fix. F260.3 must move no number, so the divergence is carried as a named worklist
//      rather than resolved: F261 landing empties `KNOWN_GATE_DIVERGENCES`, and a *seventh* entry
//      appearing fails here instead of being discovered by a user.
//
...
// `<version>|<source>:<uiKey>` for every def the matrix admits and the card hides. Each is the
// CoM2-named half of a Warlord rename, carrying `exceptVersions: ['com2_warlord_']` and writing
// the same `calcKey` as its Warlord twin. Deleting an entry is F261's job; adding one needs a
// stated reason, because an addition is a new INV-2 breach in the matrix.
const KNOWN_GATE_DIVERGENCES = [
  'com2_warlord_1.5.12.9|enchantment:flameBlade',
  'com2_warlord_1.5.12.9|enchantment:landLinking',
  'com2_warlord_1.5.12.9|enchantment:discipline',
  'com2_warlord_1.5.12.9|enchantment:destiny',
  'com2_warlord_1.5.12.9|enchantment:mislead',
  'com2_warlord_1.5.12.9|enchantment:blazingEyes',
];
...
function runVersionGateDivergenceChecks(ctx) {
  ...
  const gates = read('ABILITY_VERSION_GATES');
  const abilityVersionGated = read('abilityVersionGated');
  const subgroupAllowedForVersion = read('subgroupAllowedForVersion');
  ...
  assertSameKeyList(Object.keys(gates).sort(), ['card', 'matrix'],
    'ABILITY_VERSION_GATES names exactly the two gating tests in the tree (F260.3)');

  // --- 1. The divergence, as a worklist ---
  const divergences = [];
  for (const version of versions) {
    for (const def of abilityUiDefs()) {
      const card = gates.card(def, version);
      const matrix = gates.matrix(def, version);
      if (card === matrix) continue;
      // Only ever this direction: the matrix asks less, so it admits what the card hides. The
      // reverse would mean the matrix hides a control the card offers, which is a different bug.
      assert(card && !matrix,
        `the gate divergence on ${version}|${def.uiKey} is the matrix admitting what the card `
        + 'hides, not the reverse');
      divergences.push(`${version}|${def.source}:${def.uiKey}`);
    }
  }
  assertSameKeyList(divergences.sort(), [...KNOWN_GATE_DIVERGENCES].sort(),
    'The card and matrix version gates differ on exactly the declared worklist (INV-2). A new '
    + 'entry is a regression; a missing one means F261 landed and its KNOWN_GATE_DIVERGENCES '
    + 'entry should be deleted');

  // The card gate is `abilityVersionGated` itself, and the matrix gate is the subgroup test plus
  // the `blur` exception. Stated so a future edit to either cannot quietly redefine both.
  for (const version of versions) {
    const disagreeing = { card: [], matrix: [] };
    for (const def of abilityUiDefs()) {
      if (gates.card(def, version) !== abilityVersionGated(def, version)) {
        disagreeing.card.push(def.uiKey);
      }
      const subgroupOnly = !subgroupAllowedForVersion(def.subgroup, version)
        || (def.key === 'blur' && abilityVersionGated(def, version));
      if (gates.matrix(def, version) !== subgroupOnly) disagreeing.matrix.push(def.uiKey);
    }
    assertSameKeyList(disagreeing.card.sort(), [],
      `the card gate is abilityVersionGated for every def (${version})`);
    assertSameKeyList(disagreeing.matrix.sort(), [],
      `the matrix gate is the subgroup test plus the blur exception for every def (${version})`);
  }

  // --- 2. `applyVersionGating` ---

  // The gate is mandatory. A third caller inheriting a default silently is how the two views would
  // stop being distinguishable.
  const noGate = threwMessage(() => applyVersionGating(
    stateWithEveryControlActive(ctx, 'a'), 'mom_1.31'));
  assert(noGate !== null && noGate.includes('ABILITY_VERSION_GATES.card')
    && noGate.includes('ABILITY_VERSION_GATES.matrix'),
    'applyVersionGating halts without a gate, naming both gates');
  assert(threwMessage(() => applyVersionGating({}, 'mom_1.31', gates.card)) !== null,
    'applyVersionGating halts on a state carrying no abilities');
  assert(threwMessage(() => applyVersionGating(
    stateWithEveryControlActive(ctx, 'a'), null, gates.card)) !== null,
    'applyVersionGating halts on a missing version');

  for (const version of versions) {
    const state = stateWithEveryControlActive(ctx, 'a');
    const before = JSON.stringify(state);
    const gated = applyVersionGating(state, version, gates.card);
    assertEqual(JSON.stringify(state), before,
      `applyVersionGating leaves its argument untouched (${version})`);
    assertEqual(JSON.stringify(applyVersionGating(gated, version, gates.card)),
    ...
    for (const def of abilityUiDefs()) {
      if (!gates.card(def, version)) continue;
  ...
  for (const version of versions) {
    ...
    const gatedOfType = type => defs.find(def => gates.card(def, version) && def.type === type);
    const ungatedBool = defs.find(def => !gates.card(def, version) && def.type === 'bool');
    ...
    const sparseGated = applyVersionGating(sparse, version, gates.card);
  ...
  assertSameKeyList(Object.keys(applyVersionGating(empty, 'mom_1.31', gates.card).abilities), [],
    'applyVersionGating on a card state carrying no ability controls stays empty');

  // The gate parameter is load-bearing: under Warlord the two gates produce different states.
  const warlord = 'com2_warlord_1.5.12.9';
  const warlordState = stateWithEveryControlActive(ctx, 'a');
  const byCard = applyVersionGating(warlordState, warlord, gates.card);
  const byMatrix = applyVersionGating(warlordState, warlord, gates.matrix);
  const differing = Object.keys(byCard.abilities)
    .filter(uiKey => byCard.abilities[uiKey] !== byMatrix.abilities[uiKey]).sort();
  assertSameKeyList(differing,
    KNOWN_GATE_DIVERGENCES.map(entry => entry.split(':')[1]).sort(),
    'the two gates clear different controls under Warlord, which is why the parameter exists');
