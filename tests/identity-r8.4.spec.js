...
test('version changes remap roster identity or clear source IDs without stale special state', async ({ page }) => {
  ...
  const report = await page.evaluate(() => {
    ...
    const roster = [
      changeVersion('com2_warlord_1.5.12.9'),
...
test('special options and ordered live realm overrides preserve base identity', async ({ page }) => {
  ...
  const report = await page.evaluate(() => {
    ...
    const options = {
      ...
      warlordGolem: derive('com2_warlord_1.5.12.9', 'golem'),
      warlordChosen: derive('com2_warlord_1.5.12.9', 'chosen'),
    };
    const catapult = {
    ...
    ].map(abilities => derive('com2_warlord_1.5.12.9', 'chosen', abilities));
