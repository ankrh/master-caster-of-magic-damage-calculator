...
|---|---|---|
| 1 | **F189** | Goblin Night Goblins stop taking the Poor Vision penalty their block exempts them from. |
| 2 | **F190** | Warp Reality's unsourced Immolation To Hit arm goes, or gains a source. |
| 3 | **F206** | Blaze of Glory makes the Wall Crusher grant its block makes. |
| 4 | **F197** | Spirit Link's tooltip names Exorcise, the rider Warlord actually has. |
| 5 | **F211** | Bless's tooltip stops promising defense on channels the gate cannot admit. |
| 6 | **F196** | The eight ids that satisfy Mechanical Expert's gate become visible to the user. |
| 7 | **F181** | An unrecognised `rtbType` halts instead of deriving a unit with no attack. |
| 8 | **F131** | The vacuous Blaze of Glory distance fixture is re-aimed and `applyPreset` stops on the bad combination. |
| 9 | **F212** | Five preset `desc` fields stop stating a number or a cause the code contradicts. |
| 10 | **F214** | Two `CoM2`-named fixtures stop claiming a version they do not run. |
| 11 | **F191** | Rule on whether Rust's material-strip-only state is worth exposing. |
| 12 | **Q31** | Settle what `IsChaosUnit` tests — four calculator sites answer it differently. |
| 13 | **F195** | Spell Ward's live-Fantastic term goes. Needs Q31. |
| 14 | **F210** | The `base` phase splits into the four SPEC names. |
| 15 | **F209.1** | F202's four immovable rulings reopen. Needs F210. |
| 16 | **F209.2** | F200 stage 4's narrowing and base CoM2's empty training group reopen. Needs F210. |
| 17 | **F204** | The base training group takes `CreateUnit.CAS` line order, promoting 22 entries from provisional. |
| 18 | **Q30** + **F200 stage 3** | Sancta Basilica's over-wide Sanctify grant is settled while its two grants take positions. One subtask: each names the other as the place it is settled. Needs F210. |
| 19 | **F207** | The last published-value suppression that gates nothing goes. |
| 20 | **F180** | The four keys live in some versions and silent in others get a source reading each. |
| 21 | **F159.1** | Rule on what "same implementation in all versions" has to mean. |
| 22 | **F159.2** | Roughly 15 of the 43 unasserted keys get a scope entry with a citation. Needs F159.1. |
| 23 | **F159.3** | Roughly 15 of the 43 unasserted keys get a scope entry with a citation. Needs F159.1. |
| 24 | **F159.4** | Roughly 15 of the 43 unasserted keys get a scope entry with a citation. Needs F159.1. |
| 25 | **F215.1** | CoM2 gains a positive Black Sleep and a positive Shatter assertion. |
| 26 | **F215.2** | CoM 1 gains a positive Land Linking assertion. |
| 27 | **F216** | The Land Linking version-difference subgroup asserts a version difference. Needs F215.2. |
| 28 | **F145.1** | Family predicates are added over the existing `SCOPE_*` sets. |
| 29 | **F145.2** | Roughly 55 open-coded version tests route through the predicates and take the halt. Needs F145.1. |
| 30 | **F145.3** | Roughly 55 open-coded version tests route through the predicates and take the halt. Needs F145.1. |
| 31 | **F145.4** | Roughly 55 open-coded version tests route through the predicates and take the halt. Needs F145.1. |
| 32 | **F145.5** | Roughly 55 open-coded version tests route through the predicates and take the halt. Needs F145.1. |
| 33 | **F213.1** | The seven Warlord race-building race terms get a citation or an evidence home. |
| 34 | **F213.2** | The eight Warlord race-building hero terms get a citation or an evidence home. Needs F213.1. |
| 35 | **T8.1** | The unsourced engine claims in `combat.js` get a citation or an evidence home. |
| 36 | **T8.2** | The unsourced engine claims in `combat_special_attacks.js` get a citation or an evidence home. |
| 37 | **T8.3** | The unsourced engine claims in `combat_effects.js` get a citation or an evidence home. |
| 38 | **T8.4** | The unsourced engine claims in `combat_abilities.js` and `combat_phases.js` get a citation or an evidence home. |
| 39 | **T8.5** | The unsourced engine claims in the remaining sources get a citation or an evidence home. |
| 40 | **F153** | The modern gaze rider exclusion gets one home. |
| 41 | **F146** | The seven roster facts the card and matrix decode separately get one reader each. |
| 42 | **F147** | Six derivation values read by nothing go, F205's three write-only `luckyPhase*` markers with them. |
| 43 | **F140** | The matrix stops computing a row and column for the card that nothing renders. |
| 44 | **F151** | A version's default-state map is built without resetting the live page. |
| 45 | **F128** | The `Other` unit category no roster can fill goes. |
| 46 | **F129** | The version-scope sweep's shared attack slot gets a strength. |
| 47 | **F182** | `derivation_equivalence.js`'s dead `chaos-channels` env is dropped or fixed. |
| 48 | **F115** | The 44 Warlord spell ids the roster renders as `Spell#N` get names. |
| 49 | **T15** | The Markdown roster tools' drift from the roster JSON is fixed. |
...
| 55 | **Q1** | Troll Shaman/Magician roster values versus the manual. |
| 56 | **Q2** | The apparent Draconian common-unit +1 Resistance racial modifier. |
| 57 | **Q6** | CoM High Prayer's +3-attack text versus the +2 used elsewhere. |
| 58 | **Q8** | Whether Wraiths use Life Steal −4 or −3. |
| 59 | **Q12** | When "ranged" includes each attack kind, across versions and effects. |
| 60 | **Q15** | Whether compiled code ever consumes `unitT.savemodifier`. |
| 61 | **Q18** | What CoM 1's unit-type ceiling of `0x97` selects. F49 left it unmodelled pending this. |
| 62 | **Q19** | CoM 1 Realm Ward helptext against the executed −2/−3/−3 writes. |
| 63 | **Q21** | CoM 1 helptext's retained powers against the manual's replacements. |
| 64 | **Q25** | The R6.1h shared-source rendering disagreement at `0x8E5C6`. |
| 65 | **Q26** | The R9-G1a-R3 evidence-scope disagreement. |
| 66 | **Q28** | What realm a Fantastic unit with a mundane base race has. |
| 67 | **H1** | Contact Seravy about the Blur bug. |

Rows 1–11 are the standalone defects: each is wrong behaviour today, each is one prompt, and none
depends on the structural work below it. The deprecated backlog ordered the phase refactor first;
this does not, because F210 blocks only F209 and F200.
...
### F210 — Split the `base` phase into the phases SPEC now names

`STEP_PHASES` ([steps.js:38](Calculator/steps.js:38)) carries one `base` label with five write kinds
inside it (`BASE_WRITE_KINDS`, `steps.js:49`), so a step's position is spelled twice. SPEC's phase
table now names the real phases: `template`, `training-time`, `cast-time`, `immunity`. Make them
phases and drop the tag.

`perPass` folds into `cast-time` — SPEC's `cast-time` row already covers "modifications to the
persistent base unit that happen every time a unit performs an attack". Its two entries are
`base:destiny` and `base:destiny:supernatural`; `c:destiny` and every region-`c` step are untouched.

Delete `contestedPermanentIds` and `assertPermanentWritesLandOnce` (`steps.js:631-680`) and the call
at `:698` — the idempotence rule they enforce is no longer in SPEC. Four cases in
`tests/f20-source-order.spec.js` assert that rule and come out with it; three more retire with
`baseWrites()`. **This is the only place F210 may delete assertions.**

Otherwise mechanical: step ids (`base:x` → `training:x`), the five chain manifests, the `SCOPE_*`
rows, and the assertions keyed on `entry.phase === 'base'`. One decision: `context.base = {...unit}`
(`steps.js:746`) snapshots after every `base` step and must name which new phase closes it.

Do before F209 and F200. Versions: all five, no number moves.
...
### F204 — Put the `base` training group in `CreateUnit.CAS` line order

The Warlord chain runs its 22 `CreateUnit.CAS` steps in authoring order, not script order:
`artificer` (:37), `malnourished` (:614), `armorclad` (:700), `altarOfTheMoon` (:372),
`militaryWorkshop` (:246), `lightningBlade:breath` (:294), `poolOfRepentance` (:311), `dragonMound`
(:319), `ludusAgoge` (:335), `motherFungus` (:443), `altarOfTheSun:*` (:355),
`alumniOfAcademy:figures` (:460), `sanctaBasilica` (:412), five `naturalSelection:*` (:529…),
`pillarOfFaith` (:572), `energyCannon` (:691), `survivalInstinctToBlock` (:521).

Transcribing the real order promotes 22 entries from `provisional`. Expected to move no number —
every write is a blind addition except `lightningBlade:breath`'s assignment and `energyCannon`'s
×1.5, and both keep their relative order under either ordering. Measure rather than assume.

Two things to settle: the five `naturalSelection:*` share one block and need finer positions than
their spans give, and which line `base:armorclad` and `base:alumniOfAcademy:figures` take (their
second route is `OverlandEndTurn.CAS`). Raised in priority by F209. Warlord only.
...
One ruling first: `sanctify` gates `b:sanctify`'s conversion, which `targetingIdentity` replays on a
scratch record running the conversion list alone. Once the grant is positioned, a record read makes
that projection disagree with the sequence whenever the building grants the flag. Either widen the
projection to replay the `base` grants, or leave the gate pre-sequence and say why. F202 recommended
the first.
...
### F189 — Eternal Night's Poor Vision penalty covers a unit its block exempts

The gate is `HASGLOBAL(W,GEEternalNight)=0 %AND (GetStat(U,STypeID,1)<>356) %AND
(GetStat(U,SRace,0)<>RCDeath) %AND (GetEnchantmentFlag(U,EncUndead,0)=0)`
(`UnitCalcPre.CAS:1340-1344`). The third and fourth landed in F186; the second is unimplemented, so
Warlord template **356**, Goblin Night Goblins (Missile 5), takes the −2 it is exempt from.
`GetStat(U,S,1)` is the base unit, so this is a permanent-record read of the template id.

Settle first whether a roster exemption with no other engine consequence earns a `SPECIAL_UNIT_DEFS`
entry (`stats_identity.js`), or whether a bare template-id read belongs at the gate.

### F190 — Warp Reality's Immolation To Hit arm has no source

`toHitImmolation` (`stats.js`) re-evaluates the Warp Reality gate against Immolation's separate
spell-attack chance. But Warp Reality's block writes the *unit's* hitchance — `Dec(U.hitchance,20)`
at `$005A3E33..$005A3ED0`, `bu->tohit -= 2` at 131:0x9077A and com1:0x904DF — and the modern
damage-spell path never reads that field: `AttackRoll(str, SpellTable[sp].hitchance)` at
`$005C1696..$005C16FF` takes the *spell table's* hitchance. Immolation is spell 99 in both
`spells.ini` copies.

The DOS side is unread rather than contradictory — `BU_ProcessAttack` delivers Immolation through an
unreconstructed overlay. Decide between deleting the arm and finding a source. The calculator's own
comment two lines above says Immolation is "always base 30%, ignoring all modifiers", which the arm
contradicts. `warpRealityChaosExemptAtBlockImmolationWarlord` would need re-aiming or dropping.
...
### F196 — Name the units that satisfy Mechanical Expert's presence gate

`UnitCalc.CAS:274-300` scans the friendly side and counts a unit as an engineer/mechanic when it
carries the hero ability `HAMechanicalMaster` or its `STypeID` is one of **52, 78, 110, 117, 144,
292, 357, 363**; with at least one present, every friendly Mechanical unit gains +20% To Hit and
+10% To Defend.

The calculator collapses the scan into the boolean `mechanicalExpert` control, which is legitimate —
SPEC's one-attacker-one-defender scope cannot express a same-side presence check. But the eight ids
are written nowhere, so a user cannot tell which roster units set the tick, and Clockwork Tinmen
(363) is one of them. Name them in the control's tooltip; SPEC requires every control's tooltip to
describe its modelled effect. Warlord.

### F197 — Correct Spirit Link's tooltip

The `spiritLink` entry (`enchantments.js`) names Dispel Evil, which is MoM-only. The control is
Warlord-only, where the rider is **Exorcise** — `Caster.exe`'s `AttackFlagsT` declares an `exorcise`
member and no Dispel Evil one. Read the Bless half in the same pass;
`spiritLinkBlessNoBonusWarlord` is its home — but see F212, which finds that fixture's `desc`
states a number the engine cannot produce, and F211, which finds the Bless tooltip wrong too.

### F211 — Bless's tooltip promises defense the gate cannot admit

The `bless` entry ([enchantments.js:15](Calculator/enchantments.js:15)) says the CoM 2 and Warlord
defense applies to "fire/lightning breath, Immolation, magical Chaos ranged, and thrown or physical
ranged attacks from Chaos/Death creatures". The modern `effectiveDefense:bless` step requires
`ctx.magicImmunityEligible && ctx.spellId > 0` with a chaos or death realm
([combat_effects.js:448](Calculator/combat_effects.js:448)), and every modern **unit-attack**
descriptor passes `spellId: 0` — melee `:652`, ranged `:661`, thrown `:687`, gaze `:699`. Of the
channels the tooltip lists, only Immolation reaches the gate: its descriptor supplies `spellId: 99`,
`spellRealm: 'chaos'` and `magicImmunityEligible: true` ([combat_effects.js:707](Calculator/combat_effects.js:707)).

The attacker-realm arm the tooltip describes exists only on the DOS path (`combat_effects.js:938`,
`:958`), whose own comment records that no `com2*` version reaches it.

SPEC's *Input/output contract* requires a tooltip to describe the **modelled** effect, so this is a
spec violation rather than drift. Either narrow the tooltip to Immolation, or establish that the
gate is wrong against `Combat.ApplyAttack.pas` and fix the code instead — the item is to settle
which, not to assume the tooltip is the wrong half. Three already-declared fixtures
(`blessMeleeFromDeathCoM2`, `blessBreathBonusCoM2`, `blessBreathBonusWarlord`) assert the gate's
current behaviour, so the code and the fixtures agree and only the tooltip is the outlier.

### F206 — Blaze of Glory grants Wall Crusher and the calculator does not model it

`SETSTAT(U,AWallCrusher,0,1)` at `UnitCalc.CAS:1503` sits between the Armor Piercing and First
Strike writes already inside `PROVENANCE[blazeOfGlory]`'s reviewed span, and `d:blazeOfGlory` makes
neither the write nor a note of it. One line in the step; `wallCrusher` is already a
`POSITIONED_GRANT_WRITES` field. Open question: whether Wall Crusher should have a resolver
consequence at all, since `b:bombsGrenades` grants it too and nothing reads either.
...
### F181 — An unrecognised `rtbType` derives silently instead of halting

A shared-slot token outside `RANGED_TYPES`, `THROWN_TYPES` and `GAZE_TYPES` (`data.js`) is read as
three separate `'none'` results by `buildSlotContext` (`stats.js`), so the unit derives with no
attack in the slot rather than the run stopping. SPEC's fail-loud rule forbids that shape, and it is
what let F161's `'stoning_gaze'` typo pass a negative assertion vacuously.

Scope first: which tokens legitimately mean "no attack" (`'none'`, and the modern record whose
`rtbType` names nothing because it has no shared slot).
...
### F131 — Re-aim the vacuous Blaze of Glory distance-penalty fixture

Of 1050 presets, 243 state `rangedCheck: true` and 3 resolve melee —
`blazeOfGloryThrownTakesNoDistancePenaltyWarlord`, `focusMagicDoomGazeCoM` and
`supremeLightSkipsZeroedRangedCoM2` — because each leaves the Ranged field empty and
`refreshAbilityFieldVisibility` clears the box before `recalculate` reads it. In all three the
`rangedCheck`/`rangedDist` fields are inert.

`blazeOfGloryThrownTakesNoDistancePenaltyWarlord` is vacuous on its named axis: `distancePenaltyFor`
(`stats.js:1712`) returns 0 on `!input.rangedCheck` before it reads the finished type, so its own
counterfactual cannot fire. Re-aim it onto a case where the penalty is observable. **The boundary is
settled by SPEC's fail-loud rule**: `applyPreset` should stop on the combination rather than
normalising it silently.

Two of the three now carry `keep` declarations — `focusMagicDoomGazeCoM` from T2.16 and
`supremeLightSkipsZeroedRangedCoM2` from T2.07 — written about the gaze and ranged behaviour rather
than about `rangedCheck`, so neither contradicts this item. Re-check the reason either one carries
before touching it: a `vacuity` reason is inherited as settled.

### F212 — Five preset `desc` fields state a number or a cause the code contradicts

Found by reading fixtures against the code during T2.11–T2.23. No `expected` value is affected and
no test is red, so each is documentation drift in a fixture a later agent reads as authoritative.

| Fixture | `desc` says | The code says |
|---|---|---|
| `rustEliminatesLargeShieldWarlord` | Large Shield adds "+2 vs ranged" | `+3` ([combat_effects.js:435](Calculator/combat_effects.js:435)); `fortificationRestoresRustedLargeShieldWarlord` computes `2+3 → 7` |
| `hierophanyStripsMagicImmunityWarlord` | Magic Immunity raises def "to 50" | `u.effectiveDefense = 100` ([combat_effects.js:469](Calculator/combat_effects.js:469)) |
| `sanctaBasilicaMagicImmunityStripsWeaknessWarlord` | "both named features are live" | the named curse is inert, which is the fixture's actual point |
| `marionetteIllusionImmunityStripsMindStormWarlord` | same | same |
| `focusMagicCreationAfterLevelCoM2` | the result follows from step order | both CoM2 level gates read the frozen permanent `base` ([steps.js:746](Calculator/steps.js:746)) while Focus Magic writes only the calculated record, so reordering the two region-`c` steps changes nothing |

`spiritLinkBlessNoBonusWarlord` is a sixth of the same class but is **not** in this item: its `desc`
claims "3.0 with Bless's +7", a number no supported missile configuration can produce, which makes
it a re-aim left open by T2.20 rather than a wording fix. F211 carries the gate evidence.

The last row is the one to think about rather than retype: the fixture still tests something real —
Champion does not reach a Focus-created attack — so the question is whether its `desc` and key
should describe that, or whether it should be re-aimed onto a genuine ordering case.
